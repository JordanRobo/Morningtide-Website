<script lang="ts">
	import { EnvelopeClosed, InstagramLogo, LinkedinLogo, Mobile, TwitterLogo } from "svelte-radix";
	...
	const values = [
		...
		{
			title: "Practical",
			icon: "/images/icons/practical.png",
...
<div class="flex flex-col md:flex-row max-w-5xl mx-auto gap-4 my-12">
	...
	<div class="w-full md:w-1/2">
		<div class="space-y-1 p-4">
			<h2 class="text-3xl text-center md:text-left font-bold md:text-4xl">Our Clients</h2>
			<span class="divider mx-auto md:mx-0 divider-accent w-48"></span>
			<p class="text-center md:text-left">Schools engage with us when they:</p>
	...
	<div class="max-w-5xl mx-auto p-4 gap-4 py-8">
		...
		<div class="flex flex-wrap">
			{#each values as value}
				<div class="text-center md:text-left md:w-1/3 p-4 space-y-1">
					<img src={value.icon} alt="Icon" class="h-10 w-10 mx-auto md:mx-0" />
					<h4 class="text-2xl font-bold">{value.title}</h4>
					<p>{value.body}</p>
				</div>
			{/each}
		</div>
...
<div id="contact" class="flex flex-col max-w-3xl mx-auto p-4 gap-4 my-12">
	...
	<div class="flex flex-col items-center text-center gap-4">
		<div class="h-52 w-52 rounded-br-[96px] rounded-lg overflow-hidden shadow-primary/80 shadow-md">
			<img src="/images/uploads/Portrait_gradient-600w.webp" alt="Stuart Robinson Headshot" class="w-full h-full object-cover" loading="lazy" decoding="async" />
		</div>
		...
		<div class="card bg-primary text-white shadow-xl mt-6">
			<div class="card-body">
				<div class="flex flex-col justify-center lg:flex-row lg:justify-evenly gap-4">
					<p class="flex items-start justify-center">
						<EnvelopeClosed class="text-base-400 mx-2 h-6 w-6" />
						<span class="mx-2 truncate font-bold">info@morningtide.com.au</span>
					</p>
					<p class="flex items-start justify-center">
						<Mobile class="text-base-400 mx-2 h-6 w-6" />
						<span class="mx-2 truncate font-bold">0403 987 946</span>
					</p>
					<div class="flex justify-center mt-2 lg:mt-0">
						<a class="mx-1.5" href="https://www.linkedin.com/in/stuart-robinson-mba/" aria-label="LinkedIn">
							<LinkedinLogo class="text-white mx-2 h-6 w-6" />
						</a>
						<a class="mx-1.5" href="https://twitter.com/Morningtide_Go" aria-label="Twitter">
							<TwitterLogo class="text-white mx-2 h-6 w-6" />
						</a>
						<a class="mx-1.5" href="https://www.instagram.com/morningtide.consulting/" aria-label="Instagram">
							<InstagramLogo class="text-white mx-2 h-6 w-6" />
						</a>
					</div>
				</div>
			</div>
