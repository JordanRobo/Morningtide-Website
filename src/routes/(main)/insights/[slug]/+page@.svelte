...
<svelte:head>
    ...
    {@html `<script type="application/ld+json">${JSON.stringify(schemaArticle)}</script>`}
</svelte:head>
...
<article class="max-w-3xl mx-auto bg-white mb-4 p-8 space-y-8" transition:fade>
    <header>
        ...
        <div
            class="flex flex-col text-center md:flex-row md:justify-around my-4"
        >
            <p>{post.reading_time} min read</p>
            <p class="font-bold">By {post.primary_author.name}</p>
    ...
    <div class="card w-80 shadow-xl image-full h-full">
        ...
        <div class="card-body h-full flex flex-col justify-between">
            ...
            <div class="card-actions justify-end">
                <a href="/insights/{post.slug}" class="btn btn-primary"
                    >Read More...</a
                >
            </div>
        </div>
